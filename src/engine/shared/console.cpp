...
void CConsole::ConCommandStatus(IResult *pResult, void *pUser)
{
	...
	{
		if(pCommand->m_Flags&pConsole->m_FlagMask && pCommand->GetAccessLevel() <= clamp(pResult->GetInteger(0), (int)ACCESS_LEVEL_ADMIN, (int)ACCESS_LEVEL_USER) &&(!(pCommand->m_Flags&CMDFLAG_TEST) || g_Config.m_SvTestingCommands))
		{
