...
bool IGameController::CanSpawn(int Team, vec2 *pOutPos, CGameWorld *pWorld) const
{
	...
	if(IsTeamplay())
	{
		...
		EvaluateSpawnType(&Eval, 1+(Team&1), pWorld);
		if(!Eval.m_Got)
		{
			EvaluateSpawnType(&Eval, 0, pWorld);
			if(!Eval.m_Got)
				EvaluateSpawnType(&Eval, 1+((Team+1)&1), pWorld);
		}
	}
	else
	{
...
float IGameController::EvaluateSpawnPos(CSpawnEval *pEval, vec2 Pos, CGameWorld *pWorld) const
{
	float Score = 0.0f;
...
void IGameController::EvaluateSpawnType(CSpawnEval *pEval, int Type, CGameWorld *pWorld) const
{
	// get spawn point
	for(int i = 0; i < m_aNumSpawnPoints[Type]; i++)
	{
		...
		float S = pEval->m_RandomSpawn ? random_int() : EvaluateSpawnPos(pEval, P, pWorld);
